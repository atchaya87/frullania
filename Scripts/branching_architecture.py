import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
from sklearn.preprocessing import StandardScaler
from sklearn.decomposition import PCA

# 1. Load the data
df = pd.read_csv('Leafsnake Data.xlsx - Cleaned Data.csv')

# 2. Prepare features
features = df.drop(['ID', 'Species'], axis=1)

# 3. Standardize and PCA
X = StandardScaler().fit_transform(features)
pca = PCA(n_components=2)
coords = pca.fit_transform(X)

# 4. Plotting the main PCA
sns.set_style("whitegrid")
plt.figure(figsize=(10, 7))

sns.scatterplot(
    x=coords[:, 0], 
    y=coords[:, 1], 
    hue=df['Species'], 
    palette=["#4857ff", "#ff617b"], 
    s=120, 
    alpha=0.8,
    edgecolor='black'
)

plt.xlabel(f'PC1 ({pca.explained_variance_ratio_[0]:.1%})', fontsize=12, fontweight='bold')
plt.ylabel(f'PC2 ({pca.explained_variance_ratio_[1]:.1%})', fontsize=12, fontweight='bold')
plt.title('PCA of F. delangeii and F. rostrata based on terminal and internal branch lengths', fontsize=14, fontweight='bold', pad=20)
plt.legend(title='Species', title_fontsize='11', fontsize='10')
plt.tight_layout()

# Save PCA plot
plt.savefig('Frullania_PCA_Plot.png', dpi=300)
print("Plot saved as Frullania_PCA_Plot.png")

# Calculate Loadings
loadings = pd.DataFrame(
    pca.components_.T, 
    columns=['PC1', 'PC2'], 
    index=features.columns
)

# 5. Combined Bar Plot for Loadings
fig, axes = plt.subplots(1, 2, figsize=(12, 6))
fig.suptitle('Contribution of branching characters to PCA of F. delangeii and F. rostrata', fontsize=16, fontweight='bold', y=1.02)

loadings['PC1'].sort_values().plot(kind='barh', ax=axes[0], color='skyblue', edgecolor='black')
axes[0].set_title('Drivers of PC1', fontsize=12, fontweight='bold')
axes[0].set_xlabel('Loading Magnitude')

loadings['PC2'].sort_values().plot(kind='barh', ax=axes[1], color='salmon', edgecolor='black')
axes[1].set_title('Drivers of PC2', fontsize=12, fontweight='bold')
axes[1].set_xlabel('Loading Magnitude')

plt.tight_layout()

# Save Loadings plot
plt.savefig('PCA_Loadings_Both.png', dpi=300)
print("Loadings plot saved as PCA_Loadings_Both.png")
plt.show()

# 6. Print console analysis
print("\nTop drivers for PC1 (Species Separation):")
print(loadings.abs().sort_values(by='PC1', ascending=False)['PC1'])
print("\nTop drivers for PC2 (Secondary Variation):")
print(loadings.abs().sort_values(by='PC2', ascending=False)['PC2'])